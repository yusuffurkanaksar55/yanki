...
The applied migrations create a default-deny security foundation, user profile and invitation onboarding tables, and configurable organization hierarchy tables. They do not create evaluation submission tables or store sensitive evaluation content.

## Demo Fixtures

Synthetic CEO, HR admin, team leader, and employee accounts are described in `docs/TEST_FIXTURES.md`. The local fixture command requires `SUPABASE_SERVICE_ROLE_KEY` from the environment and is not part of normal checks.
