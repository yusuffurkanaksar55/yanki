...
- Development backend: AWS self-hosted Supabase is the canonical development and integration-test environment. Operator access remains through the loopback SSH tunnel, while the deployed production frontend uses same-origin `/supabase` behind Caddy/Nginx. Public TLS/browser acceptance is pending the AWS TCP 80/443 Security Group rule. The former Supabase Cloud project is inactive and forbidden for development/testing.
