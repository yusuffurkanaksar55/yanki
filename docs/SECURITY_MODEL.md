...
The implemented keyring selects new encryption with `EVALUATION_ACTIVE_ENCRYPTION_KEY_VERSION`. It reads the legacy `EVALUATION_ENCRYPTION_KEYRING` for backward compatibility and merges independently managed `EVALUATION_ENCRYPTION_KEY_VERSION_<VERSION>` secrets for additive rotation. Every key is exactly 32 random bytes encoded as base64. Encryption uses a fresh 12-byte nonce, a 128-bit authentication tag, and authenticated context containing tenant, cycle, optional project, subject, assignment kind, template version, and context version. Historical key secrets must remain configured while any ciphertext references them.

The AWS self-hosted development runtime uses `AWS_DEV_20260817_01` as its active version through server-only Functions environment variables. The key value is held only in the mode-`0600` server environment and its encrypted S3 backup; it is absent from Git, local frontend configuration, fixtures, and logs. Twenty imported ciphertext records still reference `DEV_20260807_01` or `development-v1`. Their key values were not found in the reviewed local/server/S3 history, so those records remain immutable and preserved as `OLD_KEY_UNAVAILABLE`; no substitute key or re-encryption is permitted. All development keys remain synthetic-only and must never be reused for production data.

`encryption-key-health` is available only to authenticated active system administrators with an exact `PLATFORM` scope and null scope id. It returns configuration validity, active-key presence, historical coverage, and total configured/referenced version counts. It never returns key material, key version names, ciphertext, evaluation content, identities, per-version usage counts, or timestamps. Organization-scoped administrators cannot read deployment-global key health.
...
API table privileges are explicit rather than dependent on Supabase project defaults. `authenticated` receives only `SELECT` on `user_profiles`, and RLS restricts that grant to the caller's own row. `service_role` receives direct CRUD only on the reviewed identity/configuration tables required by trusted Edge Functions. Encrypted submissions, anonymous credentials, retention state, bootstrap operations, abuse controls, and recovery canaries remain outside that grant and retain their narrow RPC-only or operator-only boundaries.
...
`platform-tenant-administration` is the authenticated shared-SaaS operator gateway to that same bootstrap boundary. It requires an active exact `SYSTEM_ADMIN / PLATFORM` assignment in the Edge Function and in service-role-only database wrappers. The browser never receives the service-role key, Auth action link, request fingerprint, or evaluation content. Tenant listing returns only organization and first-administrator onboarding metadata. Organization-scoped administrators cannot see the UI module or execute its database functions. The first platform operator must still be provisioned outside this UI through the reviewed trusted installation process.

Organization hierarchy records are identity-domain metadata and remain inaccessible to frontend clients. `organization-administration` validates the caller's authenticated active profile and database-backed platform or matching-organization `SYSTEM_ADMIN` role, then delegates mutations to service-role-only atomic database functions that revalidate the actor. Organization-name updates change only the display name, preserve the stable tenant slug, and audit the scope without copying the name. The database rejects manager cycles, invalid role scopes, unsafe unit archival, and removal of the final organization administrator. Demo fixture credentials must be generated at runtime and must not be committed.
...
`security-abuse-monitoring` is available only to authenticated active system administrators with an exact `PLATFORM` scope and null scope id, with authorization repeated in the Edge Function and database. It exposes 60-minute and 24-hour deployment-global aggregate counts and configured limits only. Organization-scoped administrators are denied. It cannot read credential or ciphertext tables. The separate operator summary is executable only with the service role and returns the same identifier-free global aggregates for scheduled delivery; it cannot read credentials or ciphertext.
...
Local production-container acceptance generates a fresh gateway token, verifies a direct anonymous-submission request without that token receives `403`, and then completes the encrypted browser lifecycle through Nginx. The harness accepts only loopback infrastructure, never prints the token, and removes its container, image tag, secret file, and strictly identified synthetic tenant records. This proves repository behavior, not a production environment's TLS, provider-log, secret-custody, or network configuration.
