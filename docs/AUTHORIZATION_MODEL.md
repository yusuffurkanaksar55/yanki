...
`platform-tenant-administration` requires an active exact `SYSTEM_ADMIN` assignment scoped to `PLATFORM` with a null scope id. It may list content-free tenant onboarding summaries, create a tenant through the existing idempotent bootstrap, and renew only an incomplete first-administrator invitation. The Edge Function validates the authenticated user and the service-role-only database wrappers repeat the exact platform check. Organization-scoped administrators have no direct execute grant and cannot access this module.
