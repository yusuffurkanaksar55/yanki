...
Eligible employees can submit once and database readers see ciphertext only. The content row remains directly unlinked from evaluator and assignment records, while subject and reporting scope remain available for future thresholded aggregation. The trusted credential-preparation function can observe the evaluator and raw credential during issuance, and sparse timing/context can still permit inference; therefore the system does not claim cryptographic anonymity. Production requires a new key, rotation/recovery procedures, rate limits, trusted thresholded reporting, retention policy, and backup/restore acceptance.
